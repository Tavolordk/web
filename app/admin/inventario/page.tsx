import Link from 'next/link';
import {
  AlertTriangle,
  ArrowDownCircle,
  ArrowUpCircle,
  Boxes,
  PackageCheck,
  PackageX,
  Pencil,
  Search,
} from 'lucide-react';
import { apiFetch } from '@/lib/api';
import { InventoryItem } from '@/types/product';

function getInventoryStatus(item: InventoryItem) {
  if (item.stock === 0) {
    return {
      label: 'Agotado',
      className: 'border-red-100 bg-red-50 text-red-700',
      iconClassName: 'bg-red-100 text-red-700',
      icon: PackageX,
    };
  }

  if (item.stock <= item.minStock) {
    return {
      label: 'Bajo stock',
      className: 'border-amber-100 bg-amber-50 text-amber-700',
      iconClassName: 'bg-amber-100 text-amber-700',
      icon: AlertTriangle,
    };
  }

  return {
    label: 'Disponible',
    className: 'border-emerald-100 bg-emerald-50 text-emerald-700',
    iconClassName: 'bg-emerald-100 text-emerald-700',
    icon: PackageCheck,
  };
}

function formatCurrency(value: string | number | null) {
  if (value === null) {
    return '—';
  }

  const numericValue = Number(value);

  return new Intl.NumberFormat('es-MX', {
    style: 'currency',
    currency: 'MXN',
  }).format(Number.isNaN(numericValue) ? 0 : numericValue);
}

export default async function AdminInventoryPage() {
  const inventory = await apiFetch<InventoryItem[]>('/inventory');

  const totalProducts = inventory.length;
  const availableProducts = inventory.filter((item) => item.stock > item.minStock).length;
  const lowStockProducts = inventory.filter(
    (item) => item.stock > 0 && item.stock <= item.minStock,
  ).length;
  const outOfStockProducts = inventory.filter((item) => item.stock === 0).length;

  return (
    <div className="space-y-6">
      <section className="overflow-hidden rounded-[1.7rem] border border-slate-200 bg-white p-6 text-center shadow-sm sm:p-7 xl:text-left">
        <div className="flex flex-col items-center justify-between gap-5 xl:flex-row">
          <div>
            <p className="text-xs font-extrabold uppercase tracking-[0.28em] text-blue-700">
              Operación
            </p>

            <h2 className="mt-3 text-3xl font-extrabold tracking-tight text-slate-950 sm:text-4xl">
              Inventario
            </h2>

            <p className="mt-3 max-w-2xl text-sm leading-7 text-slate-500 sm:text-base">
              Consulta existencias, mínimos, productos agotados y mercancía con
              bajo stock.
            </p>
          </div>

          <div className="grid w-full max-w-[460px] gap-3 sm:grid-cols-2">
            <Link
              href="/admin/inventario/entrada"
              className="flex min-h-[48px] items-center justify-center gap-2 rounded-2xl bg-blue-700 px-5 py-3 text-sm font-bold text-white shadow-sm transition hover:bg-blue-800"
            >
              <ArrowUpCircle size={20} />
              Entrada
            </Link>

            <Link
              href="/admin/inventario/salida"
              className="flex min-h-[48px] items-center justify-center gap-2 rounded-2xl border border-slate-200 bg-slate-50 px-5 py-3 text-sm font-bold text-slate-700 transition hover:bg-slate-100"
            >
              <ArrowDownCircle size={20} />
              Salida
            </Link>
          </div>
        </div>
      </section>

      <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <article className="flex min-h-[150px] flex-col items-center justify-center rounded-[1.5rem] border border-slate-200 bg-white p-6 text-center shadow-sm">
          <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-blue-100 text-blue-700">
            <Boxes size={24} />
          </div>
          <p className="mt-4 text-sm font-semibold text-slate-500">
            Productos en inventario
          </p>
          <h3 className="mt-2 text-3xl font-extrabold text-slate-950">
            {totalProducts}
          </h3>
        </article>

        <article className="flex min-h-[150px] flex-col items-center justify-center rounded-[1.5rem] border border-slate-200 bg-white p-6 text-center shadow-sm">
          <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-emerald-100 text-emerald-700">
            <PackageCheck size={24} />
          </div>
          <p className="mt-4 text-sm font-semibold text-slate-500">
            Disponibles
          </p>
          <h3 className="mt-2 text-3xl font-extrabold text-slate-950">
            {availableProducts}
          </h3>
        </article>

        <article className="flex min-h-[150px] flex-col items-center justify-center rounded-[1.5rem] border border-slate-200 bg-white p-6 text-center shadow-sm">
          <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-amber-100 text-amber-700">
            <AlertTriangle size={24} />
          </div>
          <p className="mt-4 text-sm font-semibold text-slate-500">
            Bajo stock
          </p>
          <h3 className="mt-2 text-3xl font-extrabold text-slate-950">
            {lowStockProducts}
          </h3>
        </article>

        <article className="flex min-h-[150px] flex-col items-center justify-center rounded-[1.5rem] border border-slate-200 bg-white p-6 text-center shadow-sm">
          <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-red-100 text-red-700">
            <PackageX size={24} />
          </div>
          <p className="mt-4 text-sm font-semibold text-slate-500">
            Agotados
          </p>
          <h3 className="mt-2 text-3xl font-extrabold text-slate-950">
            {outOfStockProducts}
          </h3>
        </article>
      </section>

      <section className="rounded-[1.8rem] border border-slate-200 bg-white p-5 shadow-sm sm:p-7">
        <div className="flex flex-col items-center justify-between gap-4 text-center xl:flex-row xl:text-left">
          <div>
            <h3 className="text-2xl font-bold text-slate-950">
              Existencias actuales
            </h3>
            <p className="mt-1 text-sm text-slate-500">
              Vista general del stock por producto.
            </p>
          </div>

          <div className="flex w-full max-w-md items-center gap-3 rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3">
            <Search size={20} className="text-slate-400" />
            <input
              type="text"
              placeholder="Buscar en inventario..."
              className="w-full bg-transparent text-sm font-medium text-slate-700 outline-none placeholder:text-slate-400"
            />
          </div>
        </div>

        <div className="mt-7 overflow-hidden rounded-2xl border border-slate-200">
          <div className="hidden grid-cols-[1.4fr_1fr_0.8fr_0.8fr_0.8fr_0.5fr] bg-slate-50 px-5 py-4 text-xs font-bold uppercase tracking-[0.14em] text-slate-500 xl:grid">
            <div>Producto</div>
            <div>Categoría / Marca</div>
            <div>Precio</div>
            <div>Stock</div>
            <div>Estado</div>
            <div className="text-right">Ajustar</div>
          </div>

          <div className="divide-y divide-slate-200">
            {inventory.length === 0 ? (
              <div className="flex min-h-[160px] items-center justify-center p-6 text-center">
                <p className="text-sm font-semibold text-slate-500">
                  Todavía no hay inventario registrado.
                </p>
              </div>
            ) : (
              inventory.map((item) => {
                const status = getInventoryStatus(item);
                const Icon = status.icon;

                return (
                  <article
                    key={item.productId}
                    className="grid gap-4 px-5 py-5 xl:grid-cols-[1.4fr_1fr_0.8fr_0.8fr_0.8fr_0.5fr] xl:items-center"
                  >
                    <div className="flex items-center gap-4">
                      <div
                        className={`flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl ${status.iconClassName}`}
                      >
                        <Icon size={24} />
                      </div>

                      <div>
                        <h4 className="font-semibold text-slate-950">
                          {item.product.name}
                        </h4>
                        <p className="mt-1 text-sm text-slate-500">
                          SKU: {item.product.sku}
                        </p>
                      </div>
                    </div>

                    <div>
                      <p className="font-semibold text-slate-800">
                        {item.product.category?.name ?? 'Sin categoría'}
                      </p>
                      <p className="mt-1 text-sm text-slate-500">
                        {item.product.brand?.name ?? 'Sin marca'}
                      </p>
                    </div>

                    <div>
                      <p className="font-bold text-blue-800">
                        {formatCurrency(item.product.price)}
                      </p>
                      <p className="mt-1 text-xs text-slate-400">
                        Costo: {formatCurrency(item.product.cost)}
                      </p>
                    </div>

                    <div>
                      <p className="font-bold text-slate-950">
                        {item.stock} pzas
                      </p>
                      <p className="mt-1 text-xs text-slate-400">
                        Mínimo: {item.minStock}
                      </p>
                    </div>

                    <div>
                      <span
                        className={`inline-flex rounded-full border px-3 py-1 text-xs font-bold ${status.className}`}
                      >
                        {status.label}
                      </span>
                    </div>

                    <div className="flex justify-start xl:justify-end">
                      <Link
                        href={`/admin/inventario/${item.productId}`}
                        className="flex h-10 w-10 items-center justify-center rounded-2xl border border-slate-200 bg-white text-slate-600 transition hover:bg-slate-50"
                      >
                        <Pencil size={18} />
                      </Link>
                    </div>
                  </article>
                );
              })
            )}
          </div>
        </div>
      </section>
    </div>
  );
}